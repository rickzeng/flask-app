...
class V2freeAutomation:
    ...
    def _get_proxy_config(self) -> Dict:
        ...
        if self.config.USE_PROXY:
            return {"server": self.config.PROXY_SERVER}
        return None
    ...
    def prepare_login_form(self) -> Dict:
        ...
        try:
            ...
            # 访问登录页面
            self.page.goto(self.config.V2FREE_URL, timeout=self.config.BROWSER_TIMEOUT)
            ...
            if has_captcha:
                ...
                if not captcha_text:
                    ...
                    for selector in captcha_img_selectors:
                        try:
                            captcha_img = self.page.wait_for_selector(
                            ...
                            if captcha_img:
                                ...
                                if box:
                                    ...
                                    filepath = os.path.join(
                                        self.config.SCREENSHOT_DIR, filename
                                    )

                                    self.page.screenshot(path=filepath, clip=box)
    ...
    def submit_login_with_captcha(self, captcha_code: str) -> Dict:
        ...
        try:
            ...
            if (
                "/user" in result["page_url"]
                or "dashboard" in result["page_url"].lower()
            ):
                result["success"] = True
                result["message"] = "登录成功"
