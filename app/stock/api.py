...
from app.stock.fetcher import StockDataFetcherHybrid
from app.stock.config import get_config, get_date_range
...
def get_top_fund_flow():
    ...
    try:
        ...
        include_details = (
            request.args.get("include_details", default="false", type=str).lower()
            == "true"
        )
        ...
        if days < 1 or days > 30:
            return jsonify(
                {"error": "参数错误", "message": "days 参数必须在 1-30 之间"}
            ), 400

        if top_n < 1 or top_n > 50:
            return jsonify(
                {"error": "参数错误", "message": "top_n 参数必须在 1-50 之间"}
            ), 400
        ...
        if output_format.lower() == "csv":
            ...
            return current_app.response_class(
                ...
                mimetype="text/csv",
                headers={
                    "Content-Disposition": f"attachment;filename=top_stocks_{datetime.now().strftime('%Y%m%d')}.csv"
                },
            )
        ...
        return jsonify({"error": "服务器错误", "message": str(e)}), 500
...
def get_stock_details(stock_code):
    ...
    try:
        ...
        if not stock_code or len(stock_code) != 6:
            return jsonify(
                {"error": "参数错误", "message": "股票代码必须是6位数字"}
            ), 400
...
def get_historical_data():
    ...
    try:
        ...
        if not stock_code:
            return jsonify(
                {"error": "参数错误", "message": "必须提供股票代码（code）参数"}
            ), 400
        ...
        # 生成模拟历史数据
        fetcher = get_stock_fetcher()
        history_data = fetcher.get_stock_history(stock_code, days=30)
        ...
        return jsonify(
            {
                ...
                "parameters": {
                    "code": stock_code,
                    "start_date": start_date if start_date else "30天前",
                    "end_date": end_date if end_date else "今天",
                    "period": period,
        ...
        return jsonify({"error": "参数错误", "message": f"日期格式错误: {str(e)}"}), 400
    except Exception as e:
...
def get_realtime_data():
    ...
    try:
        ...
        if not codes_param:
            return jsonify(
                {"error": "参数错误", "message": "必须提供股票代码（codes）参数"}
            ), 400
        ...
        if len(codes) > 20:
            return jsonify(
                {"error": "参数错误", "message": "一次最多查询20只股票"}
            ), 400
        ...
        for code in codes:
            data = fetcher.get_stock_realtime(code)
            if data:
