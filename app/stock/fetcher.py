...
from app.stock.config import get_config, get_storage_paths, get_date_range
...
class StockDataFetcherHybrid:
    ...
    def __init__(
        self, real_time_source="tencent", history_source="baostock", use_cache=True
    ):
        """
        初始化股票数据获取器（混合数据源）

        Args:
            real_time_source: 实时数据源 ('tencent', 'eastmoney', 'sina')
        ...
        self.logger.info(
            f"StockDataFetcherHybrid 初始化完成，实时数据源: {real_time_source}, 历史数据源: {history_source}"
        )
    ...
    def _convert_numpy_types(self, data: Any) -> Any:
        ...
        if isinstance(data, np.ndarray):
            ...
            return {
                key: self._convert_numpy_types(value) for key, value in data.items()
            }
    ...
    def get_stock_realtime(self, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            stock_info = {
                ...
                "change_percent": float(fields[31])
            ...
            if stock_info["preclose"] > 0:
                if stock_info["change"] == 0:
                    stock_info["change"] = (
                        stock_info["current_price"] - stock_info["preclose"]
                    )
            ...
            self.logger.info(
                f"从腾讯财经获取到 {stock_info['name']}({stock_info['code']}) 的实时数据"
            )
            return stock_info
    ...
    def get_stock_details(self, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 生成分析结论
            analysis, recommendation = self._generate_analysis(
            ...
            details = {
                ...
                "turnover_rate": realtime_data["volume"] / 100000000
                if realtime_data["volume"] > 0
                else 0,  # 简单估算
                "market_cap": realtime_data["current_price"] * 100000000,  # 简单估算
                "pe_ratio": financial_data.get("pe_ttm", 20),
                "fund_flow": fund_flow,
    ...
    def _generate_fund_flow(
        self, realtime_data: Dict, history_data: List[Dict]
    ) -> Dict:
        """生成资金流向数据（基于历史数据）"""
        try:
            # 简单的资金流向计算（基于成交量变化）
            ...
            fund_flow = {
                ...
                "large_order_inflow": large_inflow,
                "medium_order_inflow": medium_inflow,
                "small_order_inflow": small_inflow,
                "total_inflow": (
                    main_inflow + large_inflow + medium_inflow + small_inflow
                ),
            }
    ...
    def _generate_technical_indicators(self, history_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            if len(history_data) >= 2:
                ...
                ema_long = (
                    sum(d["close"] for d in history_data[-10:]) / 10
        ...
        try:
            ...
            if rs.error_code == "0":
                ...
                if len(data_list) > 0:
                    data = data_list[0]
                    return {
                        "revenue_growth": float(data[5]) if len(data) > 5 else 10.5,
                        "profit_growth": float(data[9]) if len(data) > 9 else 15.2,
                        "roe": float(data[3]) if len(data) > 3 else 12.3,
                        "debt_ratio": 0,  # Baostock 中没有这个字段
                        "pe_ttm": data[13] if len(data) > 13 else 20,
                    }
    ...
    def _generate_analysis(
        self, realtime_data: Dict, technical_indicators: Dict
    ) -> tuple:
        """生成分析结论"""
        try:
            ...
            for i, code in enumerate(popular_stocks):
                ...
                if realtime_data:
                    ...
                    stock = {
                        ...
                        "market_cap": realtime_data["current_price"]
                        * 100000000,  # 简单估算
                        "pe_ratio": 20.5,  # 示例数据
                        "fund_flow": {
                            "main_net_inflow": 50000000 + i * 10000000,
                            "large_order_inflow": 25000000 + i * 5000000,
                            "medium_order_inflow": 15000000 + i * 3000000,
                            "small_order_inflow": 10000000 + i * 2000000,
                            "total_inflow": 100000000 + i * 20000000,
                        },
    ...
    def save_to_file(self, stocks: List[Dict]) -> Optional[str]:
        """保存股票数据到文件"""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filepath = self.storage_paths["output_dir"] / f"stocks_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    fetcher = StockDataFetcherHybrid(
        real_time_source="tencent", history_source="baostock", use_cache=True
    )
    ...
    top_stocks = fetcher.get_top_fund_flow_stocks(days=5, top_n=5)
    print(f"获取到 {len(top_stocks)} 只股票:")
    for stock in top_stocks:
        print(
            f"{stock['rank']}. {stock['name']}({stock['code']}): {stock['current_price']}, 涨跌幅: {stock['change_percent']}%, 资金得分: {stock['fund_flow_score']}"
        )
